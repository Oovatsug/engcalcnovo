...
function closeSidebar() {
  ...
  setTimeout(() => { if (!sidebar.classList.contains("open")) overlay.hidden = true; }, 240);
}

/* ── Tema (claro / escuro) ── */
function getPreferredTheme() {
  const saved = localStorage.getItem("engcalc_theme");
...
function setTheme(theme) {
  try { localStorage.setItem("engcalc_theme", theme); } catch (e) {}
  applyTheme(theme);
}
...
function selectCalc(areaKey, calcId, prefill) {
  ...
  const fields = calc.fields.map(f => {
    ...
    const depAttr = f.dependsOn
      ? ` data-dep-field="${esc(f.dependsOn.id)}" data-dep-values="${esc(f.dependsOn.in.join(","))}"`
      : "";

    const control = f.type === "select"
      ? `<select id="in-${f.id}" data-select="1">
          ${f.options.map(o => `<option value="${esc(o.value)}"${String(val) === o.value ? " selected" : ""}>${esc(o.label)}</option>`).join("")}
         </select>`
      : `<input id="in-${f.id}" type="number" step="any" inputmode="decimal"
               autocomplete="off" placeholder="${esc(f.ph)}" value="${esc(val)}">`;

    return `
      <div class="field-row${f.type === "select" ? " has-select" : ""}" id="fr-${f.id}"${depAttr}>
  ...
  /* Mostra/esconde campos condicionais (ex.: resistividade do solo só quando "enterrado") */
  function updateFieldVisibility() {
    calc.fields.forEach(f => {
      if (!f.dependsOn) return;
      const row = document.getElementById("fr-" + f.id);
      const ctrl = document.getElementById("in-" + f.dependsOn.id);
      if (!row || !ctrl) return;
      row.style.display = f.dependsOn.in.includes(ctrl.value) ? "" : "none";
    });
  }
  updateFieldVisibility();
  document.querySelectorAll("#worksheet select[data-select]").forEach(sel => {
    sel.addEventListener("change", updateFieldVisibility);
  });
...
function runCalc(areaKey, calcId) {
  ...
  calc.fields.forEach(f => {
    ...
    if (!hidden && (isNaN(v) || (inp && inp.value.trim() === ""))) {
      hasError = true;
